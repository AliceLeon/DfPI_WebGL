...
const orbitalControl = new alfrid.OrbitalControl(camera, window, 50)
orbitalControl.rx.value = orbitalControl.ry.value = 0.3

window.addEventListener('touchmove', (e) => {
  const mouseX = e.touches[0].clientX / window.innerWidth
  const mouseY = e.touches[0].clientY / window.innerHeight
  socket.emit('touching', {
    x: mouseX,
    y: mouseY
  })
})
